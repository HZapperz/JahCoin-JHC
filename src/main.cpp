...
uint256 hashGenesisBlock("0x5869f60d78bc8c6ddc1e0e17dd1c999e478dfa1a2d06f4a569c2de1ed68db1fb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // JahCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "JahCoin Signed Message:\n";
...
{
    // JahCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 18 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // JahCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // JahCoin: 4 days
static const int64 nTargetSpacing = 6 * 60; // JahCoin: 6 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // JahCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x64b47b957c4955b79f4980ca99516f13e9334727ffd664dffc0baf45ec0b868b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "He gets shot";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 18 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045d5ced659114d5ff52386c2a92ad42a2afd7dd51ba1ce2b8fcc62a617fc007655b3afa35e9cb8ea8f5fc44bf4fbea9a75081e554f554af1813c72641d9060f9a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1548380042;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084616844;

        if (fTestNet)
        {
            block.nTime    = 1548379975;
            block.nNonce   = 385545098;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x42c97067c3d63bcd87fac0d89cbbb4b59ae3dbfdd1aca2ec35b8ada548593690"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xcd, 0xb3, 0xdc }; // JahCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JahCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JahCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JahCoinMiner(CWallet *pwallet)
{
    printf("JahCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("JahCoin-miner");
    ...
    try { loop {
        ...
        printf("Running JahCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JahCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JahCoinMiner, pwallet));
}
